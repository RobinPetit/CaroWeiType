  ...
  invariant := by
    ...
    · intro ⟨h, hdeg⟩
      ...
      · have H : (φ.toFun (φ.invFun φv)) = φv := by
        ...
        rw [H] at hobj
        exact hobj
      · obtain ⟨v, hv, hφv⟩ := mem_image.mp h
        rw [← hφv]
        suffices v = φ.invFun (φ v)by exact this ▸ hv
        exact Equiv.apply_eq_iff_eq_symm_apply _ |>.mp rfl
    · intro ⟨h, hdeg⟩
      refine ⟨?_, ?_⟩
      ...
      · refine fun v h ↦ le_of_eq_of_le (degree_in_eq_of_iso v s φ) (hdeg (φ.toFun v) ?_)
        simp [h]
