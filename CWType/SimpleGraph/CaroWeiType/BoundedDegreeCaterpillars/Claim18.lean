...
private lemma neighborFinset_of_adj_of_adj_of_ne {n : ℕ} {G : SimpleGraph (Fin n)}
...
private lemma hNvw {n : ℕ} {G : SimpleGraph (Fin n)} [DecidableRel G.Adj] {û v w x y : Fin n}
  ...
  simp only [closed_neighborFinset_of_Finset, mem_insert, mem_singleton, exists_eq_or_imp,
    ↓existsAndEq, true_and, mem_filter, mem_univ, ← mem_neighborFinset]
  grind [mem_neighborFinset_symm]

private lemma sum_hNvw {n : ℕ} {G : SimpleGraph (Fin n)} [DecidableRel G.Adj] {û v w x y : Fin n}
    (hdû : G.degree û = 4) (hxy : x ≠ y)
    (hdv : G.degree v = 3) (hdw : G.degree w = 3) (hvnew : v ≠ w)
    (hNv : G.neighborFinset v = {û, w, x}) (hNw : G.neighborFinset w = {û, v, y})
    {f : Fin n → ℝ} :
  ...
  if hfxy' : f G ABC x + f G ABC y ≤ 14 / 15 then
    ...
    calc (2 : ℝ)
      ...
      _ ≥ f G ABC v + f G ABC w + f G ABC û + f G ABC x + f G ABC y := by
        ...
        linarith
    exact le_of_eq <| (hNvw hNv hNw) ▸ sum_hNvw hdû hxy hdv hdw hvnew hNv hNw
  else
...
private lemma γ_eq_zero_in_N2 {n : ℕ} {G : SimpleGraph (Fin n)} [DecidableRel G.Adj]
  ...
  if hN2 : ∀ z ∈ G.N2_of_Finset {v, w}, γ G ABC z = 0 then
    ...
    obtain ⟨z, hz, hγz⟩ := hN2
    refine Or.inl <| Corollary2'' hG hBv hBw hvx.symm hwy.symm hvnew ?_ ih
    rw [hNvw hNv hNw, sum_hNvw hdû hxy hdv hdw hvnew hNv hNw]
...
lemma _respects {n : ℕ} {G : SimpleGraph (Fin n)} [DecidableRel G.Adj] {ABC : Tripartition n}
    {û v w x y : Fin n}
...
lemma _induces_forest {n : ℕ} {G : SimpleGraph (Fin n)} [DecidableRel G.Adj] {û v w x y : Fin n}
    (hwx : ¬G.Adj w x)
    (hNv : G.neighborFinset v = {û, w, x}) (hNw : G.neighborFinset w = {û, v, y}) :
    ...
    if h : G.closed_neighborFinset_of_Finset F ⊆ {û, v, w, x, y} then
      ...
      have hNx : G.neighborFinset x ⊆ {v, w, û, x, y} := by
        ...
        if hu1 : u ∈ G.closed_neighborFinset_of_Finset {v, w} then
          ...
          simp only [closed_neighborFinset_of_Finset, mem_insert, mem_singleton, exists_eq_or_imp,
            ↓existsAndEq, true_and, mem_filter, mem_univ, not_or] at hu1
          simp only [N2_of_Finset, mem_insert, mem_singleton, not_or, forall_eq_or_imp, forall_eq,
            exists_eq_or_imp, ↓existsAndEq, true_and, mem_filter, mem_univ, hu1, not_false_eq_true,
            true_and]
          refine Or.inl ?_
          refine ⟨x, ?_, ?_, ?_⟩
          · grind [degree]
      ...
      have hNwxy : G.closed_neighborFinset_of_Finset {w, x, y} = {û, v, w, x, y} := by
        ...
        · intro h
          ...
          | inl h => grind
          | inr h => ?_
          have : u ∈ G.neighborFinset w ∨ u ∈ G.neighborFinset x ∨ u ∈ G.neighborFinset y := by
            rcases h with h | h | h <;>
            simp only [G.mem_neighborFinset .. |>.mpr h.symm, mem_neighborFinset, true_or, or_true]
          grind
        · grind
      have hF : #{w, x, y} = 3 := by grind [degree]
      have hN' : ({û, v, w, x, y} : Finset _) = {v, w, û, x, y} := by grind
      rw [hNwxy, hN', sum_hNvw hdû hxy hdv hdw hvnew hNv hNw, hF, Nat.cast_three]
      rw [fB3 hBv hdv, fB3 hBw hdw, fA4 hAû hdû, fA3 hAx hdx, fA3 hAy hdy]
      linarith
