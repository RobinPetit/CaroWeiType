...
private lemma pairwise_ne {x y z : V} {s : Finset V} (hs : s = {x, y, z})
    (hs' : #s = 3) : x ≠ y ∧ x ≠ z ∧ y ≠ z := by
  exact pairwise_ne_of_triplet (hs ▸ hs')

variable [Fintype V]

private lemma deg3_neighborhood {G : SimpleGraph V} [DecidableRel G.Adj]
    {v x y : V} (hx : G.Adj v x) (hy : G.Adj v y) (hne : x ≠ y) (hdv : G.degree v = 3) :
...
@[simp, reducible]
private def _op_g (G : SimpleGraph V) (v x y z : V) : SimpleGraph V :=
  (fromEdgeSet <| G.edgeSet ∪ {s(x, y), s(x, z), s(y, z)}).deleteIncidencesOf {v}

private lemma _op_g_adj_of_notMem_vxyz {G : SimpleGraph V} [DecidableRel G.Adj]
    {v x y z u w : V} (hNv : G.neighborFinset v = {x, y, z})
    (hu : u ∈ ({x, y, z} : Finset _)) (hv : w ∉ ({v, x, y, z} : Finset _))
    (huv : (_op_g G v x y z).Adj u w) : G.Adj u w := by
  have Hu : u ∉ ({v} : Finset _) := by
    refine notMem_singleton.mpr ?_
    exact fun heq ↦ G.notMem_neighborFinset_self _ (hNv ▸ heq ▸ hu)
  obtain ⟨h, hvnew⟩ :=
    fromEdgeSet_adj _ |>.mp <| deleteIncidencesOf_adj_iff_of_notMem Hu (by grind) |>.mpr huv
  rcases Set.mem_union .. |>.mp h with h | h
  · exact G.adj_congr_of_sym2 rfl |>.mp h
  · refine hv ?_ |>.elim
    simp only [Set.mem_insert_iff, Sym2.eq, Set.mem_singleton_iff] at h
    grind

private lemma _op_g_adj_xyz {G : SimpleGraph V} [DecidableRel G.Adj] {v x y z : V}
    (hNv : G.neighborFinset v = {x, y, z}) (hdv : G.degree v = 3) :
    (_op_g G v x y z).Adj x y ∧ (_op_g G v x y z).Adj x z ∧ (_op_g G v x y z).Adj y z := by
  have : v ∉ ({x, y, z} : Finset _) := hNv ▸ (notMem_neighborFinset_self _ _)
  refine ⟨?_, ?_, ?_⟩ <;> {
    refine deleteIncidencesOf_adj_iff_of_notMem (by grind) (by grind) |>.mp ?_
    exact fromEdgeSet_adj _ |>.mpr <| by grind [degree]
  }

private lemma _op_g_deg_xyz (G : SimpleGraph V) [DecidableRel G.Adj]
    {u v x y z : V} (hx : G.Adj v x) (hy : G.Adj v y) (hz : G.Adj v z)
  ...
  suffices (_op_g G v x y z).neighborFinset u
    ...
    have hv : {v} ⊆ G.neighborFinset u :=
      singleton_subset_iff.mpr <| mem_neighborFinset .. |>.mpr <| Adj.symm <| by grind
    rw [card_sdiff_of_subset hv]
    rw [← degree, card_singleton]
    have _ : 0 < G.degree u := by
      refine G.degree_pos_iff_exists_adj _ |>.mpr ⟨v, ?_⟩
      refine mem_neighborFinset .. |>.mp <| singleton_subset_iff.mp hv
    grind
  intro w hw
  simp only [_op_g, mem_neighborFinset] at hw
  rcases adj_fromEdgeSet_union_iff.mp <| adj_of_deleteIncidencesOf_adj hw with hw' | hw'
  · refine mem_union_left _ (mem_sdiff.mpr ⟨mem_neighborFinset .. |>.mpr hw', ?_⟩)
...
private lemma C'_of_demote (ABC : Tripartition V) {u v x y z : V} (huv : u ≠ v)
...
private lemma neighborFinset_eq_outside {G : SimpleGraph V} [DecidableRel G.Adj]
    {u v x y z : V} (hu : u ∉ ({v, x, y, z} : Finset _)) (hv : G.neighborFinset v = {x, y, z}) :
  ...
  have H : G.Adj u w → w ≠ v := by
    ...
    grind
  simp only [_op_g, mem_neighborFinset]
  constructor
  · intro hwu
    refine deleteIncidencesOf_adj_of_notMem_of_notMem_of_adj (by grind) ?_ ?_
    · exact notMem_singleton.mpr <| H hwu
    · exact adj_fromEdgeSet_union_iff.mpr <| Or.inl hwu
  · intro hwu
    rcases adj_fromEdgeSet_union_iff.mp <| adj_of_deleteIncidencesOf_adj hwu with hw' | hw'
    · exact hw'
    · exact hu (by grind) |>.elim

private lemma degree_in_eq_outside {G : SimpleGraph V} [DecidableRel G.Adj]
    {u v x y z : V} (hu : u ∉ ({v, x, y, z} : Finset _)) (hv : G.neighborFinset v = {x, y, z})
    (s : Finset V) :
...
private lemma f_eq_outside {G : SimpleGraph V} [DecidableRel G.Adj]
    {ABC : Tripartition V} {u v x y z : V}
...
private lemma _support_ok {G : SimpleGraph V} [DecidableRel G.Adj]
    {ABC : Tripartition V} [ABC.Decidable] {v x y z : V}
...
lemma Claim10 {G : SimpleGraph V} [DecidableRel G.Adj] {ABC : Tripartition V}
  ...
  if hCz : ABC.C z then
    ...
    have _ : x ∈ s → y ∉ s ∧ z ∉ s := by
      ...
      constructor
      · exact notMem_of_degree_in_eq_zero_of_adj hd'x _op_g_adj_xy
      · exact notMem_of_degree_in_eq_zero_of_adj hd'x _op_g_adj_xz
    have _ : y ∈ s → z ∉ s ∧ x ∉ s := by
      intro hy
      have hd'y : (_op_g G v x y z).degree_in s y = 0 := by
        refine hsresp y hy |>.2.2 <| ?_
        refine demote_finset_from_B _ ?_ ?_
        · refine ⟨hBy, ?_⟩
          simp only [mem_singleton, Adj.ne' hvy, not_false_eq_true]
        · simp only [mem_insert, mem_singleton, true_or, or_true]
      constructor
      · exact notMem_of_degree_in_eq_zero_of_adj hd'y _op_g_adj_yz
      · exact notMem_of_degree_in_eq_zero_of_adj hd'y _op_g_adj_xy.symm
    ...
    · intro u hu
      ...
      · if huNv : u ∈ G.neighborFinset v then
          ...
          have H_degree_in : G.degree_in s u = (_op_g G v x y z).degree_in s u := by
            ...
            simp only [mem_inter]
            constructor
            · refine fun ⟨hw, hws⟩ ↦ ⟨?_, hws⟩
              refine mem_neighborFinset_deleteIncidencesOf_iff_of_notMem ?_ ?_ |>.mp
                <| G.le_fromEdgeSet_union' hw <;> grind
            · refine fun ⟨hw, hws⟩ ↦ ⟨?_, hws⟩
              have hobj := by
                refine mem_fromEdgeSet_union_neighborFinset_iff.mp
                  <| mem_neighborFinset_deleteIncidencesOf_iff_of_notMem ?_ ?_ |>.mpr hw
                <;> grind
              rcases hobj with hobj | hobj
              · exact hobj
              · grind
          if huz : u = z then
            subst huz
