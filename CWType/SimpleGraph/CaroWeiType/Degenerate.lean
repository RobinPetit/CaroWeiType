...
lemma mul_le_one {x y : ℝ} (hx : 0 < x) (hxy : y ≤ x) : y * x⁻¹ ≤ 1 := by
  calc y * x⁻¹
    _ ≤ x * x⁻¹ := by
      simp only [Right.inv_pos.mpr hx, mul_le_mul_iff_left₀]
      exact hxy
    _ = 1 := mul_inv_cancel₀ (ne_of_gt hx)
...
theorem aks_gain_decreasing (k : ℕ) (d : ℕ) (hd : k < d) :
  ...
  have := Nat.add_sub_self_right d 1 ▸ discrete_derivative_inv_eq (d + 1) d.zero_lt_succ
  rw [this]; clear this
...
lemma IsDegenerateSetIsUnionStable {V : Type*} [DecidableEq V] [Fintype V] (k : ℕ) :
    IsNonAdjacentUnionStableProp (fun G _ s ↦ @SimpleGraph.IsDegenerateSet V _ _ G _ k s) := by
  intro G _ s s' hss' h hs hs' t ht htne
  if hts : t ∩ s ≠ ∅ then
    obtain ⟨x, hx, hdxk⟩ := (hs (t ∩ s) inter_subset_right hts)
    refine ⟨x, mem_inter.mp hx |>.1, ?_⟩
    refine le_trans ?_ hdxk
    refine card_le_card ?_
    intro y hy
    refine mem_inter.mpr ⟨mem_inter.mp hy |>.1, mem_inter.mpr ⟨mem_inter.mp hy |>.2, ?_⟩⟩
    rcases mem_union.mp <| ht (mem_inter.mp hy |>.2) with hy' | hy'
    · exact hy'
    · refine (h x (mem_inter.mp hx |>.2) y |>.mt <| Decidable.not_not.mpr ?_) hy' |>.elim
      exact mem_neighborFinset .. |>.mp <| mem_inter.mp hy |>.1
  else
    simp only [ne_eq, Decidable.not_not] at hts
    have : t ⊆ s' := by
      intro x hx
      rcases mem_union.mp (ht hx) with hxs | hx
      · grind only [= mem_inter, mem_inter.mpr ⟨hx, hxs⟩]
      · exact hx
    exact hs' t this htne

lemma IsDegenerateSet_of_degree_in_le {V : Type*} [DecidableEq V] [Fintype V] (G : SimpleGraph V)
    [DecidableRel G.Adj] (k : ℕ) (s : Finset V) (h : ∀ x ∈ s, G.degree_in s x ≤ k) :
    G.IsDegenerateSet k s := by
  intro t ht htne
  obtain ⟨x, hx⟩ := nonempty_iff_ne_empty.mpr htne
  exact ⟨x, hx, le_trans (degree_in_mono ht) (h x (ht hx))⟩
...
lemma IsDegenerate_iff_induce {V : Type*} [DecidableEq V] [Fintype V]
...
theorem minDegree_le_degeneracy {V : Type*} [DecidableEq V] [Fintype V]
    (G : SimpleGraph V) [DecidableRel G.Adj] (k : ℕ) (s : Finset V) (hs : G.IsDegenerateSet k s) :
...
lemma isDegenerate_mono {V : Type*} [DecidableEq V] [Fintype V] (G : SimpleGraph V)
    [DecidableRel G.Adj] (k : ℕ) (s : Finset V) :
    G.IsDegenerateSet k s → ∀ s' ⊆ s, G.IsDegenerateSet k s' := by
  intro h s' hs' t ht htnonempty
  exact h t (subset_trans ht hs') htnonempty

@[simp]
lemma emptyset_IsDegenerate {V : Type*} [DecidableEq V] [Fintype V] (G : SimpleGraph V)
    [DecidableRel G.Adj] :
    ∀ k : ℕ, G.IsDegenerateSet k ∅ :=
  fun _ _ ht htne ↦ htne (subset_empty.mp ht) |>.elim
...
theorem DegenerateSet_mono_bounded_degree_not_in {V : Type*} [DecidableEq V] [Fintype V]
  ...
  refine ⟨x, hxt, le_trans (Finset.card_le_card ?_) hcard⟩
  intro y hy
  simp only [mem_inter] at hy ⊢
  obtain ⟨hxy, hyt⟩ := hy
  refine ⟨?_, hyt⟩
  refine mem_neighborFinset_deleteIncidencesOf_of_notMem_of_notMem_of_mem_neighborFinset ?_ ?_ hxy
  · exact notMem_singleton.mpr <| ne_of_mem_of_not_mem hyt hvt
  · exact notMem_singleton.mpr <| ne_of_mem_of_not_mem hxt hvt
...
theorem kDegenerateSetClique_iff_le_k_plus_one {V : Type*} [DecidableEq V] [Fintype V]
    (k : ℕ) (s : Finset V) [DecidableRel (completeGraph V).Adj] :
    (completeGraph V).IsDegenerateSet k s ↔ s.card ≤ k + 1 := by
  constructor
  · intro h
    if hs : s = ∅ then
    ...
    suffices (completeGraph V).neighborFinset x ∩ s = s \ {x} by
      grind
    ext v
  ...
  · intro h s' hs' hne
    ...
    simp only [degree_in]
    suffices (completeGraph V).neighborFinset x ∩ s' = (s' \ {x}) by
      calc #((completeGraph V).neighborFinset x ∩ s')
        _ = #(s' \ {x}) := by rw [this]
...
theorem AlonKahnSeymour {V : Type*} [DecidableEq V] [Fintype V] (G : SimpleGraph V)
  ...
  | zero => exact ⟨∅, by simp [IsDegenerateSet, card_eq_zero.mp hcard]⟩
  | succ m ih => ?_
  have hX' : X.Nonempty := card_pos.mp <| Nat.lt_of_sub_eq_succ hcard
  have _ := Nonempty.to_subtype hX'
  obtain ⟨v, ⟨hvX, hdegv⟩⟩ := G.exists_minimal_degree_vertex_in X
  if hδ : G.degree v ≤ k then
    obtain ⟨s', ⟨hs', hdeg', hcard'⟩⟩ :=
      ih (G.deleteIncidencesOf {v}) (X \ {v})
        (deleteIncidencesOf_support hX) (card_setminus_singleton' hvX hcard)
    refine ⟨s' ∪ {v}, ?_, ?_, ?_⟩
    · intro x hx
    ...
    · have hcard_le_k : #(G.neighborFinset v ∩ s') ≤ k :=
        le_trans (card_le_card inter_subset_left) hδ
    ...
    · have hvs' : v ∉ s' := by
      ...
      · exact aks_bound_decreasing' k
      · exact add_le_add hcard' (min_le_left ..)
  else
    simp only [not_le] at hδ
    obtain ⟨v', ⟨hv'X, hdegv'⟩⟩ := G.exists_maximal_degree_vertex_in X
    obtain ⟨s', ⟨hs', ⟨hdeg', hcard'⟩⟩⟩ :=
      ih (G.deleteIncidencesOf {v'}) (X \ {v'})
        (deleteIncidencesOf_support hX) (card_setminus_singleton' hv'X hcard)
    ...
    · refine le_trans ?_ hcard'
      have hvΔ : G.degree v' = G.maxDegree := by
        ...
        if hw : w ∈ G.support then
          ...
          exact Nat.zero_le _
      have hΔk : G.maxDegree > k := lt_of_le_of_lt' (G.degree_le_maxDegree v) hδ
      refine cw_bound_mono (aks_bound k) G hvΔ hΔk X hX ?_ ?_ ?_
      · exact aks_gain_decreasing' k
      · intro x hx
        simp_all only [gt_iff_lt]
        rw [← G.degree_eq _ hX] at hdegv
        calc k
          _ < #(G.neighborFinset x ∩ X) := by
            refine lt_of_lt_of_le hδ (hdegv x (hX ?_))
        ...
        suffices (d + 1 : ℝ)⁻¹ ≤ (d : ℝ) * (((d - 1 : ℕ) + 1 : ℝ)⁻¹ - (d + 1 : ℝ)⁻¹) by
          calc (k + 1 : ℝ) * (d + 1 : ℝ)⁻¹
            ...
            _ ≤ (d : ℝ) * ((k + 1 : ℝ) *
              ...
              linarith
        refine le_of_eq ?_
        exact Eq.symm <| avg_gain d (Nat.zero_lt_of_lt hd)
